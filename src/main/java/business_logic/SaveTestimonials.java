 ...
 * @author ANKUR
 */
public class SaveTestimonials extends HttpServlet {
		...
		try (PrintWriter out = response.getWriter()) {
			...
			if (ValidationMethods.isAlphabet(name)
				...
				try {
					...
					if (i > 0) {
						con.close();
						response.sendRedirect("English/Testimonials.jsp?msg=Thanks");
					}
				} catch (Exception e) {
					error = e.getMessage();
				}
			} else {
				response.sendRedirect("English/Feedback.jsp?msg=Invalid entry");
			}
